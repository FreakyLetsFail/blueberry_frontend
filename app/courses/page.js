...
const RustIcon = ({ size = 24, className = "" }) => (
  <svg 
    fill="currentColor" 
    width={size} 
    height={size} 
            ...
            >
                <Tab key="our-courses" title="Our Courses">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {courses.map((course) => {
                            const IconComponent = course.customIcon;
                            const cardContent = (
                                <div 
                                    className={`relative group rounded-xl p-6 border transition-all duration-300 flex flex-col items-center justify-center gap-4 h-64 ${
                                        course.active 
                                            ? "bg-zinc-900/40 border-white/10 hover:border-blue-500/50 hover:bg-zinc-900/60 cursor-pointer" 
                                            : "bg-zinc-950/50 border-zinc-900 opacity-60 grayscale hover:opacity-100 hover:grayscale-0"
                                    }`}
                                >
                                    <div className={`w-20 h-20 rounded-2xl flex items-center justify-center bg-zinc-900 border border-zinc-800 shadow-xl ${course.active ? "group-hover:scale-110 transition-transform duration-300" : ""}`}>
                                        {IconComponent ? (
                                            <IconComponent size={48} className="text-zinc-400 group-hover:text-white transition-colors" />
                                        ) : (
                                    ...
                                    <div className="text-center w-full px-2">
                                        ...
                                        {course.active ? (
                                            <div className="flex flex-col items-center gap-2 mt-2 w-full">
                                                <span className="text-xs font-medium text-blue-400 bg-blue-400/10 px-2 py-0.5 rounded-full">In Progress</span>
                                            ...
                                            <div className="flex items-center gap-1.5 justify-center mt-2 text-zinc-500">
                                                <Lock size={14} />
                            ...
                            return course.active ? (
                                <Link href="/courses/java" key={course.id}>
                                    {cardContent}
                                </Link>
                            ) : (
                        ...
                        <button className="group rounded-xl p-6 border border-zinc-900 border-dashed bg-zinc-950/30 hover:bg-zinc-900/50 hover:border-zinc-700 transition-all duration-300 flex flex-col items-center justify-center gap-4 h-64 cursor-pointer">
                            <div className="w-16 h-16 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center text-zinc-500 group-hover:text-white group-hover:border-zinc-600 transition-colors">
                                <Plus size={24} />
                            </div>
                            <span className="font-medium text-zinc-500 group-hover:text-white transition-colors">Add Course</span>
                        </button>
                    </div>
                </Tab>
                
                <Tab key="public-courses" title="Public Courses">
                    <div className="flex flex-col items-center justify-center h-64 border border-zinc-900 border-dashed rounded-xl bg-zinc-950/30">
                        <p className="text-zinc-500">Discover courses from other universities and open resources here.</p>
                        <span className="text-xs text-zinc-600 mt-2">Coming Soon</span>
                    </div>
                </Tab>
                ...
                <Tab key="private-courses" title="Private Courses">
                    <div className="flex flex-col items-center justify-center py-20 border border-zinc-900 rounded-3xl bg-zinc-950/20 max-w-4xl mx-auto w-full">
                        ...
                        <div className="flex flex-col items-center gap-8">
                            ...
                            <Button 
                                className="bg-white text-black font-bold h-12 px-10 rounded-xl hover:bg-zinc-200 transition-all"
                                isDisabled={otpValue.length !== 16}
                            >
                                Unlock Course
                            </Button>
                        </div>
